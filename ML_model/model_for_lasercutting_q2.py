...
x_train = data_train.loc()[:, ['A相电压', 'A相电流', '总有功功率', 'A相有功功率', 'A相视在功率',
                               'A相功率因素']]
y_train = data_train.loc()[:, ['工件质量1']]

x_test = data_test.loc()[:, ['A相电压', 'A相电流', '总有功功率', 'A相有功功率', 'A相视在功率',
                             'A相功率因素']]
y_test = data_test.loc()[:, ['工件质量1']]
print(y_test)
...
# 　把模型保存在本地磁盘，随时调用
gbm.save_model('model_lasercutting1.txt')  # 相当于pickel.dump序列化
gbm = lgb.Booster(model_file='model_lasercutting1.txt')  # 相当于pickel.load反序列化
y_pred = gbm.predict(x_test, num_iteration=gbm.best_iteration)
print(y_pred)
print('The rmse of prediction is:', mean_squared_error(y_test, y_pred) ** 0.5)  # rmse是均方根差
pickle.dump(gbm, open('model_lasercutting1.pkl', 'wb'))
pickle.load(open('model_lasercutting1.pkl', 'rb'))
